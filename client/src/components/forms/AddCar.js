...
import { ADD_CAR, GET_CARS,GET_CARS_BY_PERSON_ID,GET_PEOPLE } from "../../graphql/queries";
...
const AddCar = () => {
    ...
    const { refetch: refetchCars } = useQuery(GET_CARS_BY_PERSON_ID);
    ...
    const onFinish = values => {
        ...
        addCar({
            ...
            update: (cache, {data: { addCar }}) => {               
                ...
                const existing = cache.readQuery({ query: GET_CARS_BY_PERSON_ID, variables: { personId } });
                const existingCars1 = existing ? existing.carsByPersonId : [];
                cache.writeQuery({
                    query: GET_CARS_BY_PERSON_ID,
                    variables: { personId },
                    data: {
                        carsByPersonId: [...existingCars1, addCar]
                    }
                });
                setId(uuidv4())
            }
    ...
    return(
        <div style={{ display: 'flex',flexFlow: 'column', justifyContent: 'center', alignItems: 'center'}}>
            ...
            <Form name='add-car-form' layout='inline' size="large" style={{marginBottom : '40px'}} form={form}
            onFinish={onFinish}>
